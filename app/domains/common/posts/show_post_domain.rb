module Common::Posts
  class ShowPostDomain < ApplicationDomain
    def execute(post_id:)
      ...
      is_liked_parent = post.current_user_likes.any?
      is_reposted_parent = post.current_user_reposts.any?
      ...
      reply_posts_dtos = post.replies.order(created_at: :asc).map do |reply|
        is_liked_of_reply = reply.current_user_likes.any?
        is_reposted_of_reply = reply.current_user_reposts.any?
