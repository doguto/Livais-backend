class Post < ApplicationRecord
  ...
  has_many :current_user_likes, -> { where(user: Current.current_user) }, class_name: "Like", inverse_of: :post

  has_many :reposts, inverse_of: :post, dependent: :destroy
  has_many :reposting_user, through: :reposts, source: :user
  has_many :current_user_reposts, -> { where(user: Current.current_user) }, class_name: "Repost", inverse_of: :post
